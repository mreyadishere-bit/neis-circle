...
(function(){
  ...
  loadLiveData=async function(){
    ...
    const [accessRes,termsRes,blocksRes]=await Promise.all([
      ...
      state.isAdmin?sb.from('blocked_accounts').select('id,email,phone_e164,user_id,reason,created_at').order('created_at',{ascending:false}):Promise.resolve({data:[]})
    ]);
  ...
  admin=function(){
    ...
    return `${html}<section class="module-card" style="margin-top:18px"><div class="page-title"><div><h2>${tr('Account access control','التحكم في وصول الحسابات')}</h2><p>${tr('Blocking an account secures both its verified email and verified phone in the database.','يؤمّن حظر الحساب بريده الموثق ورقم هاتفه الموثق معًا في قاعدة البيانات.')}</p></div></div><div class="admin-access-grid"><form id="blockAccountForm" class="admin-access-form"><label class="field">${tr('User email','بريد المستخدم')}<input id="blockEmail" type="email" required placeholder="student@example.com"></label><label class="field">${tr('Reason','السبب')}<textarea id="blockReason" rows="3" maxlength="300" placeholder="${tr('Policy or safety reason','سبب متعلق بالسياسة أو الأمان')}"></textarea></label><button class="primary">${tr('Block email + verified phone','حظر البريد + الهاتف الموثق')}</button></form><div><h3>${tr('Blocked identities','الهويات المحظورة')}</h3><div class="blocked-list">${state.blockedAccounts.length?state.blockedAccounts.map(b=>`<div class="blocked-row"><div><b>${esc(b.email||b.user_id||'Account')}</b><small>${esc(b.phone_e164||tr('No verified phone on record','لا يوجد هاتف موثق مسجل'))}</small><small>${esc(b.reason||'')}</small></div><button class="secondary" data-unblock-account="${b.id}" data-unblock-email="${esc(b.email||'')}" data-unblock-user="${esc(b.user_id||'')}">${tr('Unblock','إلغاء الحظر')}</button></div>`).join(''):`<div class="empty"><b>${tr('No blocked identities','لا توجد هويات محظورة')}</b><span>${tr('Access restrictions will appear here.','ستظهر قيود الوصول هنا.')}</span></div>`}</div></div></div></section>`;
  };
